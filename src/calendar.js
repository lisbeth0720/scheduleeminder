...
function toGregorianFestival(m,d){
  ...
  gregorianFestival.forEach((item, index)=>{
    let str = item.split(" ");
    if(str[0] == `${m<10 ? "0"+m : m}${d<10 ? "0"+d : d}`){
      gregorian = str[1];
    }
  })
  return gregorian
...
function toLunarFestival(y,m,d){
  // 初始化农历节日数组
  ...
  if(leapMonth(y) == 12){
    if('12'+ leapDays(y) == m + '' + d ){
      lunar = '除夕'
    }
  }else{
    if('12'+ monthDays(y,12) == m + '' + d ){
      lunar = '除夕'
    }
  }
  return lunar
...
function solar2lunar (y, m, d) { // 参数区间1900.1.31~2100.12.31
  ...
  //  判断公历节日
  var gregorianFestival = toGregorianFestival(m,d)
  //  判断农历节日
  var lunarFestival = toLunarFestival(year,month,day)
  return {'lYear': year, 'lMonth': month, 'lDay': day, 'Animal': getAnimal(year), 'IMonthCn': (isLeap ? '\u95f0' : '') + toChinaMonth(month), 'IDayCn': toChinaDay(day), 'cYear': y, 'cMonth': m, 'cDay': d, 'gzYear': gzY, 'gzMonth': gzM, 'gzDay': gzD, 'isToday': isToday, 'isLeap': isLeap, 'nWeek': nWeek, 'ncWeek': '\u661f\u671f' + cWeek, 'isTerm': isTerm, 'Term': Term, 'astro': astro,'gregorianFestival':gregorianFestival,'lunarFestival':lunarFestival}
}
